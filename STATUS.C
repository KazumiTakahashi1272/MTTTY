...
void UpdateStatus(char * szText)
{
    ...
    if (szNewMsg == NULL) {
        ...
        if (strlen(szNewMsg) > dwSize) {
            LeaveCriticalSection(&gStatusCritical);
            ErrorInComm("UpdateStatusでのヒープの破損\n");
        }
    }
...
void ReportCommError()
{
    ...
    //
    if (comStat.fCtsHold)
        UpdateStatus("Tx は CTS信号を待機している.\r\n");

    if (comStat.fDsrHold)
        UpdateStatus("Tx は DSR信号を待機している.\r\n");

    if (comStat.fRlsdHold)
        UpdateStatus("Tx は RLSD信号を待機している.\r\n");

    if (comStat.fXoffHold)
        UpdateStatus("Tx待機中、XOFF文字が読込まれました。\r\n");

    if (comStat.fXoffSent)
        UpdateStatus("Tx待機中、XOFF文字が送信されました。\r\n");
    
    if (comStat.fEof)
        UpdateStatus("EOF文字を受信しました。\r\n");
