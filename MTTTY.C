...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    if (!VersionCheck()) {
        MessageBox(NULL, "MTTTYは、このバージョンのWindowsでは実行できません。", NULL, MB_OK);
        return 0;
    }

    if (!InitializeApp(hInstance, nShowCmd)) {
        MessageBox(NULL, "MTTTYを起動できませんでした！", NULL, MB_OK);
        return 0;
    }
...
void OpenTTYChildWindow(HWND hWnd)
{
    ...
    if (ghWndTTY == NULL)
        ErrorReporter("TTYチャイルドウィンドウを作成できません");
